...
if "%MODELROOT%"=="" (
  ...
  echo Usage: build_workflow_v35.bat "E:\hydrological_modeling\ModelRoot" [output-folder]
  exit /b 2
)
...
if errorlevel 1 (
  echo [ERROR] UnifiedHydroWorkflow V3.5 build failed.
  exit /b 6
)
...
echo [OK] UnifiedHydroWorkflow V3.5 built:
