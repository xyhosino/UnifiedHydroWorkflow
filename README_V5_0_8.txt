...
版本定位
------------------------------------------------------------
V5.0.8 基于 V5.0.7 稳定版开发。
本修改版完善帮助、运行诊断和更新检查，同时修正若干界面/兼容性遗留问题。
...
6. 修正缺少 Workflow Core 时仍显示“Workflow V3.2”的旧提示，改为当前 Core V3.4。
7. AppInfo.cs 集中维护 V5.0.8、Workflow Core V3.4、GitHub 和 Release 地址。
8. Windows 批处理继续使用 BOM-free ASCII + CRLF，避免 CMD 解析异常。

保持不变
------------------------------------------------------------
- GIS 自动检测优先级仍为：ArcGIS Pro → ArcMap → QGIS。
- Workflow Core 仍为 V3.4。
...
然后：
  ...
  .\prepare_deployment_v508.bat "E:\hydrological_modeling\ModelRoot"
...
版本号规则
------------------------------------------------------------
正式用户版本号固定使用最多三段：V5.0.8、V5.0.9、V5.1.0。
不使用四段用户版本号。
