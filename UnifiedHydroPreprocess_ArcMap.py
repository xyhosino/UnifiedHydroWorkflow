...
strategy as the ArcGIS Pro backend. The hydrological core is not modified.
"""
...
def _to_path_text(value):
    """Return a Unicode filesystem path on Python 2 so Chinese names use Win32 wide APIs."""
    if isinstance(value, text_type):
        return value
    if isinstance(value, bytes):
        encodings = []
...
def log(message):
    ...
    if sys.version_info[0] < 3:
        try:
            print(message.encode("mbcs", "replace"))
        except Exception:
            print(repr(message))
    else:
        print(message)
...
def has_field(fc, target_name):
    ...
    return any(field.name.upper() == target for field in arcpy.ListFields(fc))
...
def _remove_shapefile_bundle(shp_path):
...
def _move_shapefile_bundle(source_shp, target_shp):
    """Rename an ASCII ArcPy result to the final Unicode name at filesystem level.

    ArcGIS Desktop/Python 2 can mis-handle a UTF-8 byte-string output basename such as
    土壤类型.  Keeping the geoprocessing output name ASCII and doing the final rename
    with Unicode paths avoids that code-page ambiguity without changing the data.
    """
    source_base, _ = os.path.splitext(source_shp)
    target_base, _ = os.path.splitext(target_shp)
    moved = 0
    for extension in (".shp", ".shx", ".dbf", ".prj", ".cpg", ".sbn", ".sbx", ".shp.xml"):
        ...
        if os.path.exists(source):
            ...
            moved += 1
    if moved == 0 or not os.path.exists(target_base + ".shp"):
        raise IOError(u"ArcGIS Desktop 裁剪结果重命名失败：{} -> {}".format(source_shp, target_shp))


def smart_clip(in_data, clip_lyr, out_base_path, temp_tag):
    desc = arcpy.Describe(in_data)
    datatype = str(desc.dataType)
    if datatype not in ("FeatureClass", "ShapeFile"):
        raise ValueError(u"ArcGIS Desktop 预处理仅接受矢量 SHP 土壤/土地利用：{}".format(in_data))

    out_base_path = _to_path_text(out_base_path)
    out_file = out_base_path + u".shp"
    out_dir = os.path.dirname(out_file)
    temp_file = os.path.join(out_dir, u"__uh_{}_clip.shp".format(temp_tag))

    _remove_shapefile_bundle(temp_file)
    _remove_shapefile_bundle(out_file)
    if arcpy.Exists(temp_file):
        arcpy.Delete_management(temp_file)
    if arcpy.Exists(out_file):
        arcpy.Delete_management(out_file)

    # Use an ASCII temporary basename for ArcGIS Desktop/Python 2 geoprocessing.
    arcpy.Clip_analysis(in_data, clip_lyr, temp_file)
    if not arcpy.Exists(temp_file) and not os.path.exists(temp_file):
        raise IOError(u"ArcGIS Desktop 裁剪没有生成输出：{}".format(temp_file))

    _move_shapefile_bundle(temp_file, out_file)
    return out_file
...
def dbf_schema_signature(shp_path):
    ...
    with open(dbf_path, "rb") as stream:
        ...
        while stream.tell() < header_length:
            ...
            if not descriptor or _byte_int(descriptor[0]) == 0x0D:
                break
            raw_name = descriptor[0:11].split(b"\x00", 1)[0]
            field_type = chr(_byte_int(descriptor[11]))
            field_width = _byte_int(descriptor[16])
            decimal_count = _byte_int(descriptor[17])
    ...
    for raw_name, field_type, field_width, decimal_count in schema:
        try:
            ...
            try:
                ...
                name = "".join("{:02x}".format(_byte_int(ch)) for ch in raw_name)
    ...
    try:
        ...
        try:
            for wscu in units:
                ...
                out_soil = smart_clip(soil_data, wata_lyr, os.path.join(folder_path, u"土壤类型"), "soil")
                out_land = smart_clip(landuse_data, wata_lyr, os.path.join(folder_path, u"土地利用"), "land")
...
def main(argv=None):
    ...
    try:
        ...
        return fail("无法导入 arcpy。请使用 ArcGIS Desktop / ArcMap 自带的 Python 2.7 环境运行：{}".format(exception), 3)
